...
{
    std::cout << "\r" << Colorizer::BOLD_GREEN << "[Norn]" << Colorizer::RESET << ": " << log;
}

void
CompilerOutputs::Warn(std::string log)
{
    std::cout << "\r" << Colorizer::BOLD_YELLOW << "[Norn WARN]" << Colorizer::RESET << ": " << log;
}

void
CompilerOutputs::Fatal(std::string log)
{
    std::cout << "\r" << Colorizer::BOLD_RED << "[Norn FAIL]" << Colorizer::RESET << ": " << log << std::endl;
    std::exit(38);
}
